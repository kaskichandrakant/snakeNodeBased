...
function updateFoodPos(foodPos, head) {
  if (head[0] == food[0] && head[1] == food[1]) {
    score += 10
    printFood(food, ' ');
    foodPos[0] = Math.floor(+(Math.random(1) * lastPositionX).toFixed(0)+1)
    foodPos[1] = Math.floor(+(Math.random(1) * lastPositionY).toFixed(0)* -1+2)
    snake.push(last);
  }
  printFood(foodPos, '@');
}
...
function moveSnake(headIndex, update) {
